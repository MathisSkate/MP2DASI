...
if (strlen($inputLienAction) == 0) {
    $inputLienAction = null;
} else {
    $lien = $inputLienAction;

    if (strpos($lien, ".be/")) {
        $inputLienAction = str_replace(".be/", "be.com/v/", $lien);
    } else if (strpos($lien, "watch?v=")) {
        $inputLienAction = str_replace("/watch?v=", "/v/", $lien);
    } else if (strpos($lien, "embed")) {
        $inputLienAction = str_replace("embed", "v", $lien);
    } else {
        $inputLienAction;
    }
}
...
if (isset($inputNomAction)) {
    ...
    $stmt = $bdd->prepare("UPDATE action SET id_action='$action', nom_action='$inputNomAction', media_action='$inputLienAction' where id_action = '$action' ");
...
//Permet d'ajouter des fichiers PDF
//Vérifie si il y a déjà des media existant dans l'actualité
$reqCountMedia = $bdd->query("SELECT id_media FROM illustrer WHERE id_action=" . $action . " ORDER BY id_media DESC LIMIT 1 ");
$nbMedia = $reqCountMedia->fetch();

//Si oui on insert en ajoutant l'id au nom POUR ne pas remplacer le fichier existant.
if ($nbMedia[0] > 0) {
    insertMediaAction($bdd, $_FILES['fichiers']['name'], 'action', $action);
} else {
    insertMediaAction($bdd, $_FILES['fichiers']['name'], 'action', $action);
}
