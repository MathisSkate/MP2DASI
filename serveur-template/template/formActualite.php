...
if ($lastPage != "actualites.php"){
    ...
    $reqActu = $bdd->query("SELECT type_actualite, titre_actualite, description_actualite, resume_actualite, date_actualite, id_utilisateur_detail FROM actualite WHERE id_actualite = ".$idActu." ");
...
?>
<main class="col-lg-10 froalaTextarea <?php if ($modif){echo "modifActu-btn";} else echo "ajoutActu-btn";?>">
    ...
    <section id="form_actu">
        <form enctype="multipart/form-data" method="post">
            ...
            <div class="form-group">
                ...
                <input type="date" name="dateActualite" id="dateActualite"
                       class="form-control" placeholder="Date de l'actualité" value="<?php if ($modif){echo $actualite['date_actualite'];}?>" required>
            </div>
            <div class="form-group">
                <label><?php echo $q[$_SESSION['lang']]['formActualite']['g']?></label>
                <textarea id="editor" name="descriptionActualite" required><?php if ($modif){echo $actualite['description_actualite'];}?></textarea>
            </div>
