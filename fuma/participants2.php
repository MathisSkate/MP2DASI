...
<main class="col-lg-10 pageAdminUser">
    ...
    <section class="content-container row">
        <div class="col-lg-12">
            <h2><?php echo $q[$_SESSION['lang']]['participants']['a']?></h2>
        </div>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-end">
                    ...
                    for ($i = 10; $i <= ($valeur+10) ; $i = $i+10){
                        ...
                        if(( $i % 10 ) == 0){
                            echo "<li class=\"page-item\"><a id=\"".($i-9)."\" class=\"pagePart page-link\" href=\"#\">".$page."</a></li>";
                        }
                    }?>
                </ul>
            </nav>
        ...
        <div class="col-lg-12">
            <table id="UsersParticipants" class="w-100 table table-responsive table-striped text-center">
                <thead>
                <tr>
                ...
                <tbody class="tbody-table-participants">
                    <?php
                        $req = $bdd->query("select * from commune.utilisateur as tab1 join fuma.utilisateur_detail as tab2
                                                      on tab1.id_utilisateur = tab2.id_utilisateur");
                        //var_dump($req[0]);
                        foreach ($req as $row){
                            ?>
                            <tr>
                                <td><img src="../media_commun/avatar/<?php echo $row["photo_utilisateur"] ?>"></td>
                                <td><?php echo $row["nom_utilisateur"] ?></td>
                                <td><?php echo $row["prenom_utilisateur"]?></td>
                                <td><?php echo $row["statut_utilisateur"]?></td>
                                <td><?php echo $row["etablissement_utilisateur"]?></td>
                                <td><?php echo $row["mail_utilisateur"]?></td>
                                <td> <a href="profil.php?mail=<?php echo $row["mail_utilisateur"]?>">Lien vers le profil</a> </td>
                            </tr>
                            <?php
                        }
                    ?>
