...
if (isset($inputNomAction)) {
    ...
    //affecter le numero
    $query=$bdd->prepare('SELECT * from fuma.action;');
    $query->execute();
   


    $stmt = $bdd->prepare("INSERT INTO fuma.action(numero_action, nom_action,media_action)VALUES (?,?,?)");
    //$stmt->bindParam(1, $derniereAction);
    ...
    if ($nbSousAction == 0) {
       ...
       // $stmt->bindParam(1, $idSousAction);
        ...
        for ($i = 0; $i < $nbSousAction; $i++) {
            ...
            $stmt = $bdd->prepare("INSERT INTO fuma.sous_action(num_sous_action, nom_sous_action, id_action)
            VALUES (?, ?, ?)");
            //$stmt->bindParam(1, $idSousAction);
